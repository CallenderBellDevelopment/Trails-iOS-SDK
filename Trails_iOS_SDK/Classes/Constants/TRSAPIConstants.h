//
//  TRSAPIConstants.h
//  Trails_iOS_SDK
//
//  Created by Georgi Christov on 16/06/2015.
//  Copyright (c) 2015 Callender Bell Development. All rights reserved.
//

#ifndef Trails_iOS_SDK_TRSAPIConstants_h
#define Trails_iOS_SDK_TRSAPIConstants_h

#define API_APP_URL @"http://192.168.1.41:8888"
//#define API_APP_URL @"http://192.168.0.8:8080/TriaServerTomcat"
//#define API_APP_URL @"http://graffitab.com"
#define API_APP_API_URL @"api/rest"

#define API_USER_LOGIN                            @"/user/login"
#define API_USER_CHECK_LOGIN                      @"/user/checkLogin"
#define API_USER_GET_ALL                          @"/user/getAll"

#define API_TRACK_GET_ALL                         @"/track/getTracks"
#define API_TRACK_PLAY                            @"/track/play"

#endif
